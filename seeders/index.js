...
async function seedUser () {
  try {
    let user = await User.findOne({
      'username':'Ernestina.Douglas68',
      'isDeleted':false,
      'isActive':true
    });
    if (!user || !user.isPasswordMatch('McQ6YL0lIuo_3TB') ) {
      let user = new User({
        'password':'McQ6YL0lIuo_3TB',
        'username':'Ernestina.Douglas68',
    ...
    if (!admin || !admin.isPasswordMatch('qM3GgRo4H3M_uHQ') ) {
      let admin = new User({
        'password':'qM3GgRo4H3M_uHQ',
        'username':'Camilla79',
...
async function seedUserRole (){
  try {
    ...
    if (user && user.isPasswordMatch('McQ6YL0lIuo_3TB') && userRole){
    ...
    if (admin && admin.isPasswordMatch('qM3GgRo4H3M_uHQ') && adminRole){
      let count = await UserRole.countDocuments({
        userId: admin.id,
        roleId: adminRole.id
      });
